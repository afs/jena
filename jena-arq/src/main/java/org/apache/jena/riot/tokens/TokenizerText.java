...
//            case CH_VBAR:       reader.readChar(); token.setType(TokenType.VBAR);      /*token.setImage(CH_VBAR);*/ return token;
    ...
    // Previously seen high surrogate waiting for a low surrogate - or NO_CODEPOINT
    private int previousCodePoint = NO_CODEPOINT;

    // Development checking. Compiled away if false.
    private static final boolean CheckStartFinishPairing = false;

    private static enum MODE { NONE, U, NU }
    ...
    private int insertCodepoint(int previousCP,int ch) {
        if ( Character.charCount(ch) == 1 ) {
            ...
            // Surrogate waiting?
            if ( CHECK_CODEPOINTS && previousCodePoint != NO_CODEPOINT ) {
